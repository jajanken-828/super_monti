...
// ─── DELETE LOGIC (SPECIFIC BOX ONLY - NO SHIFTING) ──────────────────
const deleteSection = (sectionId) => {
    ...
    // UPDATED: Simply filter out the specific ID. Coordinates of other boxes remain unchanged.
...
const triggerConfirm = (title, message, confirmText, type, action) => { confirmConfig.value = { title, message, confirmText, type, action }; showConfirmModal.value = true; };
const openUseModal = (item) => { selectedStockItem.value = item; materialForm.quantity = item.quantity; showUseModal.value = true; };
const openDetailsModal = (item) => { detailItem.value = item; showDetailsModal.value = true; };

const submitUse = () => {
    triggerConfirm('Release Material', `Execute release of ${materialForm.quantity} units?`, 'Confirm', 'green', () => {
        showConfirmModal.value = false;
        materialForm.post(route('warehouse.monitor.use', selectedStockItem.value.id), { onSuccess: () => { showUseModal.value = false; router.reload(); } });
    });
};
        ...
        <Teleport to="body">
            ...
            <div v-if="showUseModal" class="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/40 backdrop-blur-sm animate-in fade-in duration-300">
                <div class="bg-white rounded-[2.5rem] shadow-2xl w-full max-w-[380px] overflow-hidden animate-in zoom-in duration-200 uppercase font-black p-8">
                    <h3 class="text-lg text-slate-900 tracking-tighter mb-6">Floor release</h3>
                    ...
                    <div class="space-y-4 font-black uppercase">
                        <div>
                            <label class="text-[8px] text-slate-400 block mb-1">Quantity</label>
                            <input v-model="materialForm.quantity" type="number" step="0.01" class="w-full p-4 bg-slate-50 border-none rounded-xl font-black text-lg text-center" />
                        </div>
                        <div>
                            <label class="text-[8px] text-slate-400 block mb-1">Destination</label>
                            <select v-model="materialForm.manufacturing_department" class="w-full appearance-none p-4 bg-slate-50 border-none rounded-xl text-xs text-center font-black uppercase">
                    ...
                    <div class="flex gap-2 mt-8">
                        ...
                        <button @click="submitUse" class="flex-[2] py-4 bg-emerald-600 text-white rounded-xl text-[10px] shadow-lg">Release</button>
                    </div>
                </div>
            </div>
